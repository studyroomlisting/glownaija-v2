...
const SUGGESTED_PROMPTS = [
  { emoji: '🎀', text: 'How do I care for 4C natural hair?' },
...
export default function GlowAIChatPage() {
  const [messages,   setMessages]   = useState<Message[]>([{ ...WELCOME, time: nowTime() }])
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    const userMsg: Message = { role: 'user', content: trimmed, time: nowTime() }
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: data.response, time: nowTime() }])
    } catch {
  ...
  return (
    <div className="bg-page chat-viewport-height">
      <div className="h-full grid lg:grid-cols-[1fr_320px] gap-4 p-4">
        ...
        {/* ══ LEFT: Chat panel ══════════════════════════════════════════ */}
        <div className="flex flex-col bg-white rounded-2xl border border-bdr overflow-hidden shadow-sm min-h-0">

          {/* Header */}
          <div className="border-b border-bdr px-4 py-3 flex items-center justify-between flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 min-h-0">
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-sm md:max-w-xl ${msg.role === 'user' ? 'items-end' : 'items-start'} flex flex-col`}>
                  ...
                  {msg.time && <span className="text-2xs text-ink-3 mt-1 px-1">{msg.time}</span>}
                </div>
            ...
            {/* Suggested prompts — inline for mobile/tablet only (desktop has the persistent sidebar) */}
          ...
          <div className="border-t border-bdr px-4 py-3 flex-shrink-0">
            <form onSubmit={handleSubmit} className="flex gap-2 items-center bg-page-2 rounded-2xl border-2 border-transparent focus-within:border-rose px-2 transition-colors">
              <span className="text-rose text-sm pl-2 flex-shrink-0">✨</span>
              <textarea
                ref={inputRef}
                value={input}
                ...
                maxLength={MAX_CHARS}
                rows={1}
                className="flex-1 min-w-0 bg-transparent px-1 py-3 text-sm outline-none resize-none leading-relaxed disabled:opacity-50"
                style={{ minHeight: '24px', maxHeight: '120px' }}
              />
              <button type="button" tabIndex={-1} aria-label="Attach file (coming soon)" title="Coming soon"
                className="w-8 h-8 flex items-center justify-center text-ink-3 hover:text-rose transition-colors flex-shrink-0 disabled:opacity-30" disabled>
                📎
              </button>
              <button type="button" tabIndex={-1} aria-label="Add emoji (coming soon)" title="Coming soon"
                className="w-8 h-8 flex items-center justify-center text-ink-3 hover:text-rose transition-colors flex-shrink-0 disabled:opacity-30" disabled>
                🙂
              </button>
              <button
                ...
                className="w-10 h-10 rounded-full bg-rose text-white flex items-center justify-center flex-shrink-0 my-1.5 mr-1 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-rose-dark transition-colors"
            ...
            <div className="flex items-center justify-between mt-2 px-1">
              <p className="text-2xs text-ink-3">Press Enter to send · Shift+Enter for new line</p>
              {charCount > 400 && (
                <span className={`text-2xs ${charCount >= MAX_CHARS ? 'text-rose font-bold' : 'text-ink-3'}`}>
                  {charCount}/{MAX_CHARS}
                </span>
              )}
            </div>
            <p className="text-2xs text-ink-3 text-center mt-1">
              ✓ Glow AI can make mistakes — always verify important advice
            </p>
          </div>
        </div>

        {/* ══ RIGHT: Popular Questions ══════════════════════════════════ */}
        <div className="hidden lg:flex flex-col bg-white rounded-2xl border border-bdr overflow-hidden shadow-sm min-h-0">
          <div className="px-4 py-3 border-b border-bdr flex-shrink-0">
            <p className="font-bold text-sm flex items-center gap-1.5">✨ Popular Questions</p>
          </div>
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {SUGGESTED_PROMPTS.map(({ emoji, text }) => (
              <button key={text} onClick={() => sendMessage(text)} disabled={loading}
                className="w-full flex items-center gap-3 text-left px-4 py-3 bg-page-2 hover:bg-rose-50 border border-transparent hover:border-rose/30 rounded-xl transition-all text-sm font-medium text-ink-2 group disabled:opacity-50">
                <span className="text-base flex-shrink-0">{emoji}</span>
                <span className="flex-1 group-hover:text-rose transition-colors">{text}</span>
