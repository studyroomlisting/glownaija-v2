...
export interface TestimonialItem { text: string; name: string; initial: string; meta?: string; rating?: number }
...
export default function Testimonials({ testimonials = DEFAULT_TESTIMONIALS }: { testimonials?: TestimonialItem[] }) {
        ...
        <div className="card card-body text-center flex-1">
          ...
          <div className="flex items-center justify-center gap-2.5">
            ...
            <div className="text-left">
              ...
              {t.meta ? <p className="text-xs text-ink-3">{t.meta}</p> : <p className="text-gold text-xs">{'★'.repeat(t.rating || 5)}</p>}
            </div>
          </div>
        </div>
