...
export default function CartItem({ item }: { item: CartItemType }) {
  ...
  function increment() {
    const result = updateQty(item.id, item.quantity + 1)
    if (result?.error) { setWarning(result.error); setTimeout(() => setWarning(''), 3000) }
  }
    ...
    <div className="flex gap-4 py-4 border-b border-bdr last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={increment} disabled={item.quantity >= item.stock}
            className="w-7 h-7 rounded-lg border-2 border-bdr flex items-center justify-center text-sm font-bold hover:border-rose hover:text-rose transition-colors disabled:opacity-40 disabled:hover:border-bdr disabled:hover:text-ink disabled:cursor-not-allowed">
            +
          </button>
        ...
        {warning && <p className="text-2xs text-rose font-semibold mt-1">{warning}</p>}
      </div>
      {/* Price */}
